...
ad_proc -public qf_get_inputs_as_array {
    ...
    quotes all input values. Use ad_unquotehtml to unquote a value.
    Returns 1 if form inputs exist, otherwise returns 0.
    <br>
    If <code>duplicate_key_check</code> is 1, checks if an existing key/value
    pair already exists, otherwise just overwrites existing value. (default 0)
    <br>
    If <code>multiple_key_as_list</code> is 1, returns a list of values for
    duplicate (and multiple) referenced keys instead of whatever
    duplicate_key_check is set to do.
    <br>
    If <code>hash_check</code> is 1,
    confirms that input is from one instance of a form generated by q_form 
    by confirming that unique hash passed by form is the same as the hash
    generated at time form was generated.
    <br>
    If <code>post_only</code> is 1, confirms that input is via cgi POST.
    Ignores input via GET.
    ...
    set arg_arr(multiple_key_as_list) 1
...
ad_proc -public qf_select { 
    ...
    # 'value' is used to pass option list to qf_options
    lappend attributes_tag_list value
    ...
    foreach {attribute value} $arg_list {
        ...
        if { $attribute_index > -1 } {
            ...
            if { [lsearch -exact $attributes_tag_list $attribute_lc] > -1 } {
                ...
                ns_log Notice "qf_select.670: Ignoring attribute ${attribute_lc} value '${value}'. Not in attributes_tag_list '${attributes_tag_list}'"
            }
        } else {
    ...
    if { [info exists attributes_arr(tabindex) ] } {
        ...
        foreach option_att_list $attributes_arr(value) {
            ...
            set oa_names_list [list ]
            foreach {n v} $option_att_list {
                set nlc [string tolower $n]
                set n_arr(${nlc}) $n
                set option_att_arr(${nlc}) $v
                lappend oa_names_list $n
            }
            if { ![info exists option_att_arr(${tabindex_c}) ] } {
                set $option_att_arr(${tabindex_c}) ""
            }
            if { [info exists option_att_arr(selected) ] } {
                if { $option_att_arr(selected) ne 1 } {
                    set option_att_arr(${tabindex_c}) $unselected
                }
            } elseif { [info exists option_att_arr(checked) ] } {
                if { $option_att_arr(checked) ne 1 } {
                    set option_att_arr(${tabindex_c}) $unselected
                }
            } else {
                set option_att_arr(${tabindex_c}) $unselected
            ...
            set new_option_att_list [list ]
            foreach nlc $oa_names_list {
                lappend new_option_att_list $n_arr(${nlc}) $option_att_arr(${nlc})
            }
    ...
    foreach attribute $attributes_list {
        ...
        } elseif { [string match -nocase "value" $attribute] } {
            # skip value, but use it for qf_option further on.
        } else {
            lappend tag_attributes_list $attribute $attributes_arr(${attribute})
        }
    }
